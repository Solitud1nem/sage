...
## 2026-08-10 — sage-chat.pages.dev: чат-поверхность демо (визуал из sage-canton) — `release`

Портирован чат-фид из `D:\Canton project\sage-canton\frontend` на Sage/Monad-стек: **`apps/chat/`** (ванильный статик: index.html + app.js + styles.css, ~50KB) на отдельном Pages-проекте **`sage-chat.pages.dev`**. Lifecycle рана как диалог: бриф → Orchestrator предлагает явный план (персоны агентов ✍️🏗️🧪📦🔎📥🧠⚖️ из живого V2-реестра, цены в WMON) → **Edit plan** (locked-template семантика M13.1.1: spec/executor/цена правятся, структура и evaluator-связки read-only, живой пересчёт тотала) → Approve → SSE-баблы (акцепты, verdict со скриншотом inline, settlement с Monadscan-ссылками, dispute/council/refund) → финал с Live-preview + site.zip / отчётом. Транспорт: gateway `?chain=monad` (plan-endpoints + execute + SSE), поллинга нет. Гарды: pre-flight сверка стоимости плана со спонсорским балансом (после живого фейла run `4f02a12c` — createTask реверт при нехватке WMON), обрезка viem-простыней в баблах (+full error в details). CORS: `sage-chat.pages.dev` добавлен в gateway (`544846a6`) и Monad-оркестратор. **Проверено вживую четырьмя ранами**: фейл-кейс нехватки WMON (поймано, UX починен), зелёный website (4/4 paid, QA PASS 91 со скриншотом «Vera Sokolova» в ленте, preview+zip), зелёный research (7/7 paid), и **чистый failure-demo целиком через UI** (run `692906d7`, ~9.5 мин, без дропов): 5 шагов paid → fact-checker FAIL score 0 с перечнем нерезолвящихся цитат → council-руллинг «client» с reasoning в ленте → **рефанд-бабл с tx** → переделка → второй FAIL → второй руллинг → второй рефанд → «The protocol said no». Synthesizer (task #47) не получил ничего, fact-checker оплачен за оба вердикта. Полный нарратив «деньги не уходят за непроверенную работу» теперь показывается в чате от брифа до рефанда — скринкаст-готово. Грабля в GOTCHAS: `pnpm exec wrangler pages deploy` из папки без package.json заливает весь монорепо. Хвост: DNS `chat.sage.xyz` поверх — когда появится sage.xyz (M9-бэклог).

## 2026-08-10 — M14 закрыт: полный Sage-стек live на Monad testnet, e2e включая on-chain рефанд — `chain` `release`

Той же сессией после деплоя контрактов (ниже) — **весь стек поднят и проверен e2e**. **14.2 SDK**: `monadTestnet`-конфиг + `ChainConfig.settlement` (symbol/decimals/permit), approve-путь (`approve.ts`, exact-amount + receipt-чек; wiring в оба escrow-клиента — plan-runner ходит v1!), decimals-аудит (fail-loud `assertMoneyEnvForSettlement`, `formatUsdc(decimals)`), газ: ноль ручных лимитов — viem estimate ≈ used. Гейты 44/477/28 + root lint. **14.3 Fly**: `sage-demo-agents-monad` + `sage-workers-monad` (8 identities), machine-limit 20/20 → снесены 2 Base-standby-дубля (одобрено). Регистрация 8 identities (цены 0.15–1.0 WMON). **14.4 gateway** `d0a8fade`: `?chain=monad`-маршрутизация, индексер (fromBlock 52461965, чанки ≤100), `BASE_INDEX_DISABLED=true` (крон вернулся, Alchemy молчит). **14.5 web** Pages `5860fbbf`: пилюля Monad, `settlementOf()`, WMON-суммы во всех composite-компонентах; browser-verified. **14.6 e2e**: website `5c7cf429` **SETTLED 187s** (vs ~315s Base!) — превью/zip живые, 4/4 Paid; research `0940d9d5` SETTLED 219s, fact-check pass, отчёт живой; **failure-demo: рефанд ДОКАЗАН on-chain** — synthesizer #23 **Refunded(5)** после fail→dispute→council, fact-checker #24 Paid за вердикты. Три новых GOTCHAS: artifact-store env на новом чейн-аппе (ран №1 деградировал в legacy-approve — потеря 1.95 WMON), спонсорский газ ~1 MON/ран (limit-модель), dropped-tx→viem-timeout (rework #25 завис — зачистка `reclaim-task25-monad.sh`, retry-follow-up в BACKLOG). Экономика сессии: ~9.5 WMON эскроу-оборота + ~4 MON газа. Открыто: скор-чек Monad-индексера (~3ч бэкфилла), опц. чистый повторный failure-ран, `wrangler` обновить (v3→v4 warning).

## 2026-08-10 — M14.0–14.1: Sage контракты live на Monad testnet — `chain` `release`

Той же сессией, что ADR-0026 (ниже). **Recon 14.0** (всё сверено живыми RPC): chain-id 10143, RPC `testnet-rpc.monad.xyz`, **CreateX уже задеплоен** на каноническом адресе (гэп recon закрыт), WMON `0xFb8bf4c1…BC541` (18 dec, **без permit** — DOMAIN_SEPARATOR/nonces → `0x`, approve-путь подтверждён), gas ~102 gwei (списание по **лимиту**), `eth_getLogs` чанк ≤100 блоков; recon §5. **Деплой 14.1** (Alex, one-shot `script/deploy-monad-testnet.sh`; ключ не покидал терминал оператора): **AgentRegistryV2 `0x8df78599868Ec740C26F0eb0b660519b166cDd9e`** — адрес **побайтово = Base** (ADR-0001 инвариант на третьем чейне; блок 52461940) + **TaskEscrowV2(WMON) `0xcc01a4F195f9c991A7BEB2c513cc30267fFfdAac`** (новый скрипт `DeployEscrowWmon.s.sol`, соль `sage:escrow-wmon:v1`; блок 52461965 = fromBlock индексера). Кост ~0.456 MON из 20. Смоуки зелёные (token=WMON, owner/arbiter, registry пуст), sourcify **exact_match** оба. Ранбуки: `deploy-monad-testnet.md` + `monad-testnet-verification-2026-08-10.md`. Далее — M14.2 (SDK: chain-config, approve-путь, decimals-аудит 18v6, gas-лимиты).

## 2026-08-10 — ADR-0026: Monad testnet deployment (WMON, approve-path, gas-limit policy) — `adr` `chain`

Переориентация на Monad testnet под грант-питч (`docs/market/`: whitepaper + дека). Решения Alex: объём — **полный demo-стек** (протокол + gateway + агенты + web, оба пайплайна e2e), settlement — **WMON** (тот же байткод TaskEscrowV2, соль `sage:escrow-wmon:v1`; permit уже в try/catch → approve-путь в SDK без контрактных изменений). Три Monad-специфики зафиксированы: WMON без EIP-2612, газ списывается по `gas_limit`, CreateX на testnet не подтверждён. Главный риск порта — decimals 18 vs 6 (аудит USDC-допущений — M14.2.3). Base-прод остаётся в спячке; gateway-деплой вернёт cron — Base-индексация гейтится env-флагом (M14.4.2). Разбивка — `TASKS.md` Milestone 14 (14.0 recon → 14.6 e2e-приёмка: website + research + failure-demo с on-chain рефандом на Monad). Прецедент — Arc-порт (ADR-0015). Перед этим в сессии: аудит питч-материалов против Base-реальности (главные находки: «no admin keys» опровергается кодом — Ownable/arbiter-EOA; «live today» vs спячка; internal staking-док рядом с публичными материалами).
