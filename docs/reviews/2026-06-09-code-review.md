...
- ✅ **CR.14 (закрыт 2026-06-10), все 6:** generation-token'ы в `use-demo-stream`/`use-wallet-demo` — счётчик поколений run'а, stale continuations (поздний fetch-resolve, SSE-хендлеры, polling-циклы) no-op'ятся; в wallet-хуке это заменило общий `cancelledRef`, который новый `start()` сбрасывал в `false` и «оживлял» старый in-flight run. `chain: null` → реальный viem-chain в обоих wallet-mode `writeContract` (viem теперь enforce'ит сеть кошелька). «Depends on» — локальный draft-state, парсинг на blur/Enter (раньше controlled re-render съедал запятые при вводе). `STATUS_COLORS` в subtask-drawer: + `awaiting-review` `#A78BFA`, + `refunded` `#F59E0B`, `disputed` выровнен на `#A78BFA` по палитре `plan-graph.tsx`. Hardcoded `5042002` убран из composite-демо (`ARC_TESTNET_CHAIN_ID`/`BASE_MAINNET_CHAIN_ID`, типы `SageChainId`/`CompositeChainId`). `SAGE_CHAINS`/`wagmiConfig` Arc-drift закрыт: `arcTestnetChain` (`defineChain`, зеркало `apps/demo-agents/src/shared/config.ts`) добавлен в wagmi chains+transports. Typecheck + build чистые. Примечание: `next lint` падал на pre-existing typed-linting конфиг-проблеме — заведён и закрыт CR.15 (ниже).
- ✅ **CR.15 (закрыт 2026-06-10):** lint-гейт восстановлен для всего репо — сломан был не только web: корневой `eslint` падал тем же `await-thenable` (typed-правила из `recommended-type-checked` без `parserOptions`), т.е. lint-гейт из AGENTS.md фактически не работал. Корневой `.eslintrc.json` получил `projectService: true` + `disable-type-checked`-override для `*.test.ts`/`*.config.ts`; web мигрирован с deprecated `next lint` на ESLint CLI с flat-конфигом (`eslint.config.mjs`: FlatCompat `next/core-web-vitals` + typed-правила на ts/tsx); корневой `pnpm lint` теперь chained с web (CI покрывает оба). Все ~85 вскрывшихся нарушений починены (string-throws → `Error` в server.ts, void-обёртки async-хендлеров/сигналов, enum-typed getTask-cast'ы, floating/misused promises в web, unknown-параметризация JSON.parse и пр.). Гейты: оба lint'а 0 ошибок, `pnpm -r typecheck` чистый, demo-agents 211/211, adapter-evm 37/37, web build чистый.
