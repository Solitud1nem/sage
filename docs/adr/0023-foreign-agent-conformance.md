...
6. **Quarantine + per-agent ceilings** (Layer 3.8–3.9) — once reputation exists to graduate agents out of quarantine. — **value-ceiling DONE 2026-06-24 (M13.2.4)**: `plan-guards.checkQuarantine` at `/execute` — an unproven foreign agent (foreign + fewer than `QUARANTINE_PROVEN_MIN` settled tasks in the reputation index) may only run a sub-task `≤ QUARANTINE_MAX_UNITS` (default 0.1 USDC); proven foreign + first-party have no ceiling. Opt-in (off until `FIRST_PARTY_AGENTS` set). Cold-start ramp: a new agent earns its track record on cheap work. **Deferred:** per-day frequency/spend ceiling (Layer 3.9) — needs per-agent daily accounting; the per-sub-task value cap is the high-value half.
