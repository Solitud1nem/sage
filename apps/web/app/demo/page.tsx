...
import { useDemoStream, type AgentMode } from '@/hooks/use-demo-stream';
 ...
 *
 *   Two orthogonal axes:
 *     mode       watch | wallet     → who pays (sponsor or BYO-wallet)
 *     agentMode  pipeline | sentiment | vision → what the demo does
 *
...
export default function DemoPage() {
  ...
  // Reset both hooks on every mode/agentMode switch so UI state doesn't leak.
  useEffect(() => {
    watch.reset();
    wallet.reset();
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-6 md:px-10 py-14">
      ...
      <p className="mt-5 max-w-[720px] text-[16px] leading-[1.55] text-text-muted">
        Pick an agent — pipeline, sentiment, or vision — and watch the four-step settlement replay
        on-chain. Start in <span className="text-text font-medium">Watch live</span> mode to see it
        without signing, or switch to <span className="text-text font-medium">Try with wallet</span>{' '}
        to escrow USDC and approve yourself.
      </p>
